...
/**
 * Setup script for Feature #105: Push notification 1h before appointment
 *
 * Creates a test appointment ~1 hour from now with a client that has a user account.
 * The user account needs to have push subscriptions registered to receive push notifications.
 * The cron endpoint should find this appointment and send a push reminder.
 */
async function main() {
  console.log("=== Setup for Feature #105: Push Notification 1h ===\n");
  ...
  if (!client) {
    ...
    const [newClient] = await db.insert(clients).values({
      salonId: salon.id,
      firstName: "PUSH_TEST",
      lastName: "REMINDER_F105",
      phone: "+48123456789",
      email: "pushtest105@example.com",
    }).returning();
    client = newClient;
  ...
  const [appointment] = await db.insert(appointments).values({
    ...
    endTime,
    status: "scheduled",
    notes: "TEST_F105_PUSH_REMINDER",
    reminderPush1hSentAt: null,
  }).returning();
  ...
  console.log(`  ReminderPush1hSentAt: null (not yet)`);
  ...
  console.log(`  4. Trigger the cron with: POST /api/cron/push-reminders`);
  console.log(`  5. You should receive a push notification`);
